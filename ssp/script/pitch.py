...
def main():
    ...
    #w = np.hanning(fs)
    w = core.gaussian(fs)
    f = core.Frame(a, size=fs, period=fp)
    f = core.ZeroMean(f)
    wf = core.Window(f, w)
    p = core.Periodogram(wf)

    # Plot
    fig = plot.Figure(5,1)
    #pSpec = fig.subplot()
    #core.specplot(pSpec, p, r)
    if True:
        sSpec = fig.subplot()
        specPCM = core.PulseCodeModulation(hiPitch*2)
        fig.specplot(sSpec, p[:,:pcm.hertz_to_dftbin(hiPitch, fs)], specPCM)

    method = core.parameter('Method', 'ac')

    if method == 'ac':
        if False:
            # Low order AR
            order = 3
            a = core.Autocorrelation(p, 'psd')
            la, lg = ar.ARRidge(a, order, ridge=0.8)
            f = ar.ARExcitation(wf, la, lg)
            f = core.ZeroMean(f)
            p = core.Periodogram(f)

        # Autocorrelation method, loosely after Boersma
        ac = core.Autocorrelation(p, 2, 'psd')
        for i in range(len(ac)):
            ac[i] /= ac[i, 0]
        wac = core.Autocorrelation(w)
        wac /= wac[0]
        nac = core.Divide(ac, wac)
        ...
        m = core.Argmax(nac, loACBin, hiACBin)
        ...
        # Low order AR
        order = 15
        a = core.Autocorrelation(wf)
        la, lg = ar.ARLevinson(a, order)
        f = ar.ARExcitation(f, la, lg)

        # High order AR
        order = 150
        fh = core.Window(f, np.hanning(fs))
        ep = core.Periodogram(fh)
        a = core.Autocorrelation(fh)
        a, g = ar.ARLasso(a, order, 500)
        l = ar.ARSpectrum(a, g, nSpec=fs/2)

        epSpec = fig.subplot()
        lSpec = fig.subplot()
        core.specplot(epSpec, ep[:,:ep.shape[1]/2+1], pcm)
        core.specplot(lSpec, l, pcm)
        ...
        if 1:
            ...
            f = core.Parameter("Frame", 10)
            zplot(fig, c[f])

    elif method == 'map':
        h = core.Harmonogram(p, 'psd')
        hSpec = fig.subplot()
        core.specplot(hSpec, h, pcm)

        # Low order AR
        order = 15
        a = core.Autocorrelation(f)
        la, lg = ar.ARLevinson(a, order)
        e = ar.ARExcitation(f, la, lg)

        eh = core.Harmonogram(e)
        ehSpec = fig.subplot()
        core.specplot(ehSpec, eh, pcm)

        frame = core.Parameter('Frame', 1)
        rSpec = fig.subplot()
        rSpec.set_xlim(0, hiDFTBin-1)

        rSpec.plot(np.divide(p[frame,:hiDFTBin], core.Norm(p[frame,:hiDFTBin], 2)), 'c')
